...
export default function Login() {
    ...
    const handleLogin = (e: React.FormEvent) => {
        ...
        let sessionUser = { id: '', username: '', role: 'HEAD_IT' };

        // Skenario 1: Akun Admin Master
        if (inputUser === 'admin' && password === 'admin12345') {
            sessionUser = { id: 'admin', username: 'Admin Master', role: 'ADMIN' };
            targetRoute = '/dashboard-admin';
        }
        // Skenario 2: Akun Master Ariana (default demo)
        else if (inputUser === 'ariana.17200' && password === '12345678') {
            sessionUser = { id: 'ariana', username: 'Ariana', role: 'HEAD_IT' };
            targetRoute = '/dashboard';
        }
        // Skenario 3: Cek user yang didaftarkan Admin via UserContext (localStorage)
        else {
            const rawUsers = localStorage.getItem('itticketing_users');
            let found = false;
            if (rawUsers) {
                try {
                    const allUsers = JSON.parse(rawUsers) as Array<{
                        id: string; username: string; password: string; role: string; name: string; status: string;
                    }>;
                    const matched = allUsers.find(
                        u => u.username.trim().toLowerCase() === inputUser && u.password === password
                    );
                    if (matched) {
                        found = true;
                        ...
                        sessionUser = {
                            ...
                            role: matched.role === 'Head IT' ? 'HEAD_IT' : 'STAFF_IT_LEADER'
                        };
                        targetRoute = sessionUser.role === 'HEAD_IT' ? '/dashboard' : '/dashboard-staff';
                    }
                } catch (_) { /* ignore */ }
            }
            if (!found) {
    ...
    const EyeIcon = ({ isVisible, onClick }: { isVisible: boolean; onClick: () => void; }) => (
        <button type="button" onClick={onClick} className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-blue-600 transition duration-300">
            {isVisible ? (
    ...
    return (
        <div className="h-screen w-full relative overflow-hidden flex items-center justify-center px-6 font-sans bg-[#F3F8FC]">
            ...
            <div className="fixed top-0 left-0 w-[150vw] h-screen z-[100] pointer-events-none flex animate-wave-sweep">
                ...
                <svg viewBox="0 0 500 1000" className="h-full w-[50vw] text-[#2563EB] fill-current" preserveAspectRatio="none">
            ...
            <div className="absolute bottom-0 left-0 w-[200%] z-0 pointer-events-none opacity-95 animate-wave">
                <svg viewBox="0 0 1440 320" className="w-full h-[240px] md:h-[280px]" preserveAspectRatio="none">
                    <defs>
                    ...
                    <path fill="url(#waveGrad)" filter="url(#waveShadow)" d="M0,220 C240,320 480,120 720,210 C960,300 1180,160 1440,230 L1440,320 L0,320 Z" />
                </svg>
            </div>

            {/* ================= LOGO ================= */}
            <div className="absolute top-6 left-6 md:top-8 md:left-10 z-20">
                <div className="cursor-pointer hover:scale-[1.02] transition-transform duration-300" onClick={() => navigate('/')}>
            ...
            <div className="relative z-10 w-full max-w-[1200px] flex flex-col lg:flex-row items-center justify-between gap-8 md:gap-12">
                ...
                <div className="hidden lg:flex w-1/2 items-end justify-center relative pt-8 animate-slide-right">
                    <img src={LoginAsset} alt="IT Support Illustration" className="w-[85%] max-w-[580px] object-contain drop-shadow-[0_20px_40px_rgba(30,58,138,0.08)] hover:scale-[1.01] transition-all duration-700 ease-in-out animate-float" />
                </div>

                {/* ================= RIGHT SECTION (LOGIN CARD) ================= */}
                <div className="w-full lg:w-1/2 flex flex-col items-center justify-center animate-fade-in">
                    
                    <h1 className="text-[38px] md:text-[46px] font-extrabold text-slate-800 tracking-tight mb-6 text-center opacity-0 animate-slide-up-fade" style={{ animationDelay: '0.1s' }}>
                        Selamat Datang
                    </h1>
                    ...
                    <div className="w-full max-w-[435px] bg-white/75 backdrop-blur-2xl rounded-[32px] border border-white/80 shadow-[0_20px_50px_rgba(30,58,138,0.05)] px-8 py-8 md:px-10 md:py-10 relative">
                        ...
                        {showErrorPopup && (
                            <div className="absolute inset-0 z-50 flex items-center justify-center bg-white/95 backdrop-blur-md rounded-[32px] animate-scale-up">
                                <div className="p-6 flex flex-col items-center w-[95%] relative">
                                    <button onClick={() => setShowErrorPopup(false)} className="absolute top-2 right-2 text-slate-400 hover:text-rose-500 p-2 rounded-full transition-colors">
                                    ...
                                    <h3 className="text-xl font-bold text-slate-800 text-center tracking-tight">Login Gagal</h3>
                        ...
                        <form onSubmit={handleLogin} className="space-y-5">
                            {/* USERNAME */}
                            <div className="opacity-0 animate-slide-up-fade" style={{ animationDelay: '0.2s' }}>
                                <label className="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2 ml-3">User Name</label>
                            ...
                            {/* PASSWORD */}
                            <div className="opacity-0 animate-slide-up-fade" style={{ animationDelay: '0.3s' }}>
                                <label className="block text-[12px] font-bold text-slate-500 uppercase tracking-widest mb-2 ml-3">Password</label>
                                <div className="relative">
                                    <input 
                            ...
                            <div onClick={() => setRememberMe(!rememberMe)} className="flex items-center gap-2.5 ml-3 cursor-pointer select-none group w-fit opacity-0 animate-slide-up-fade" style={{ animationDelay: '0.4s' }}>
                                <div className={`w-4 h-4 rounded-[5px] border flex items-center justify-center transition-all duration-300 ${rememberMe ? 'bg-blue-600 border-blue-600 shadow-sm' : 'border-slate-300 group-hover:border-blue-400'}`}>
                                    {rememberMe && (
                                ...
                                <span className="text-[13px] font-semibold text-slate-500 group-hover:text-slate-700 transition-colors">Ingat saya</span>
                            </div>

                            {/* LOGIN BUTTON */}
                            <button type="submit" className="opacity-0 animate-slide-up-fade w-full h-[52px] mt-2 rounded-full bg-blue-600 hover:bg-blue-700 text-white text-[16px] font-bold shadow-[0_8px_20px_rgba(37,99,235,0.2)] hover:shadow-[0_12px_24px_rgba(37,99,235,0.3)] transition-all duration-300 active:scale-[0.98]" style={{ animationDelay: '0.5s' }}>
                                Log In
                            </button>
                        </form>

                        {/* ================= FOOTER ================= */}
                        <div className="mt-8 pt-5 border-t border-slate-100 opacity-0 animate-slide-up-fade" style={{ animationDelay: '0.6s' }}>
                            <div className="flex justify-between items-center">
                                ...
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-[11px] font-bold text-slate-400">IT Helpdesk System</span>
                                </div>
                            </div>
                            ...
                            <div className="mt-4 bg-blue-50 border border-blue-100 rounded-2xl px-4 py-2.5 flex items-start gap-2.5">
                                ...
                                <p className="text-[11px] font-semibold text-blue-500 leading-relaxed">
                                    Pendaftaran akun dilakukan oleh <strong>Admin</strong>. Hubungi Admin IT Anda jika belum memiliki akun.
                                </p>
                            </div>
                        </div>
                    </div>
