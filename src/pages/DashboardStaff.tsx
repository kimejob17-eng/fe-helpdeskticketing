...
export default function DashboardStaff() {
    ...
    const totalInProgress = tickets.filter(t => t.status !== 'Completed').length;
    ...
    const chartData = tickets
        ...
        .map(t => ({
            name: `T-${t.id}`,
            poin: t.pointsEarned,
            color: t.pointsEarned > 0 ? '#22c55e' : t.pointsEarned < 0 ? '#ef4444' : '#f59e0b'
        }));
    ...
    const getStatusStyle = (status: string) => {
        switch (status) {
            ...
            case 'Recheck': return 'bg-slate-50 text-slate-600 border-slate-200';
            ...
            <div className={`fixed md:relative z-50 h-full ${isSidebarOpen ? 'translate-x-0 w-64' : '-translate-x-full w-64 md:translate-x-0 md:w-20'} bg-gradient-to-b from-blue-600 via-blue-600 to-indigo-700 shadow-2xl transition-all duration-300 ease-in-out flex flex-col shrink-0 border-r border-blue-500/30`}>
                ...
                <div className="flex-1 py-6 flex flex-col gap-2.5 px-3.5 overflow-y-auto overflow-x-hidden">
                    ...
                    >
                        ...
                        <span className={`whitespace-nowrap text-[13px] tracking-wide uppercase transition-all duration-300 ${isSidebarOpen ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-4 hidden'}`}>PROFILE</span>
                    </div>
                ...
                <div className="md:hidden bg-white/80 backdrop-blur-md px-6 py-4 flex items-center justify-between shadow-sm z-30 border-b border-slate-100">
                    ...
                    <div className="flex items-center gap-3">
                        <div className="text-right hidden sm:block">
                            <p className="text-xs font-black uppercase tracking-widest text-[#3B82F6]">Staff Dashboard</p>
                ...
                <div className="px-6 md:px-10 py-8 flex-1 overflow-y-auto custom-scrollbar z-10">
                    <div className="max-w-[1200px] mx-auto">
                        ...
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <div className="hidden md:flex items-center gap-4 bg-white/60 backdrop-blur-sm px-4 py-2 rounded-2xl border border-slate-200/60 shadow-sm">
                                <div className="text-right">
                                    ...
                                    <p className="text-[14px] font-extrabold text-slate-800 mb-0.5">{currentUser?.username}</p>
                                </div>
                                <div className="w-12 h-12 rounded-full border-2 border-white shadow-md overflow-hidden bg-white">
                                    <img src={getMyAvatar()} alt="Profile" className="w-full h-full object-cover" />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
                            ...
                            <div className="lg:col-span-2 grid grid-cols-2 gap-5">
                                <div className="col-span-2 bg-gradient-to-r from-blue-600 to-blue-800 p-6 rounded-2xl shadow-sm flex flex-col justify-center hover:shadow-md transition-shadow relative overflow-hidden group">
                                    ...
                                    <p className="text-[12px] font-semibold text-blue-100 uppercase tracking-widest mb-1 relative z-10">Total Poin Tiket Keseluruhan</p>
                                    <p className="text-4xl font-bold text-white relative z-10 flex items-center gap-2">⭐ {tickets.reduce((sum, t) => sum + (t.pointsEarned || 0), 0)}</p>
                                </div>
                                ...
                                <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-center hover:shadow-md transition-shadow">
                                    ...
                                    <p className="text-3xl font-bold text-amber-500">{totalReopen}</p>
                                </div>
                                <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-center hover:shadow-md transition-shadow">
                                    <p className="text-[12px] font-semibold text-slate-500 uppercase tracking-widest mb-1">Sedang Berjalan</p>
                                    <p className="text-3xl font-bold text-blue-600">{totalInProgress}</p>
                                </div>
                            </div>
                            
                            {/* Graphic / Chart */}
                            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col items-center justify-center">
                                <p className="text-[12px] font-semibold text-slate-500 uppercase tracking-widest mb-4">Grafik Poin Per Tiket</p>
                                {chartData.length > 0 ? (
                                    <div className="w-full flex-1 min-h-[220px] pb-2">
                                        <ResponsiveContainer width="100%" height="100%" className="border-none outline-none">
                                            <BarChart data={chartData} margin={{ top: 20, right: 10, left: -20, bottom: 10 }}>
                                                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#F1F5F9" />
                                                <XAxis dataKey="name" tick={{ fontSize: 11, fontWeight: 700, fill: '#94A3B8' }} axisLine={false} tickLine={false} dy={10} />
                                                <YAxis tick={{ fontSize: 11, fontWeight: 700, fill: '#94A3B8' }} axisLine={false} tickLine={false} dx={-10} />
                                                <Tooltip 
                                                    cursor={{ fill: '#F8FAFC' }}
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center">
                                        ...
                                        <p className="text-[12px] font-bold text-slate-400">Belum ada data grafik</p>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5 z-10 relative">
                            {tickets.length > 0 ? tickets.slice((currentPage - 1) * ITEMS_PER_PAGE, currentPage * ITEMS_PER_PAGE).map(t => (
                                <div key={t.id} className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 flex flex-col justify-between hover:shadow-md transition-shadow relative overflow-hidden z-10">
                                    <div className={`absolute right-0 top-0 w-2 h-full ${t.status === 'Completed' ? 'bg-emerald-500' : t.status === 'In Progress' ? 'bg-blue-500' : 'bg-rose-500'}`}></div>
                                        ...
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                ...
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    {t.date && (
                                                        <span className={`px-2.5 py-0.5 rounded text-[10px] font-black border uppercase tracking-wider ${getPriorityBadgeStyle(calculateDynamicPriority(t.date, t.status, t.completedAt, t.reopenCount > 0), t.status)}`}>
                                            ...
                                            <div className="flex flex-col items-end gap-1.5">
                                                ...
                                                {t.pointsEarned !== undefined && (
                                                    <span className="text-[11px] font-black text-amber-600 bg-amber-50 px-2.5 py-1 rounded-md border border-amber-200 shadow-sm flex items-center gap-1">
                                                        ⭐ {t.pointsEarned > 0 ? `+${t.pointsEarned}` : t.pointsEarned} Poin
                                                    </span>
                                                )}
                                            </div>
                                        </div>
