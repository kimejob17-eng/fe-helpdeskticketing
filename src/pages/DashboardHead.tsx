...
export default function DashboardHead() {
    ...
    const Speedometer = ({ level }: { level: 'LOW' | 'Medium' | 'HIGH' }) => {
        const rotation = level === 'LOW' ? '-rotate-45' : level === 'Medium' ? 'rotate-0' : 'rotate-45';
        return (
    ...
    return (
        <div className="flex h-screen bg-[#EEF5FF] font-sans overflow-hidden relative">
            ...
            <div className="flex-1 flex flex-col h-screen overflow-hidden relative z-10">
                ...
                {/* Header Bar */}
                <header className="h-20 bg-white/70 backdrop-blur-md border-b border-slate-100 shadow-sm flex items-center justify-between px-8 shrink-0">
                    ...
                    >
                        <div className="text-right hidden sm:block">
                            <p className="text-slate-800 font-extrabold text-xs leading-none">{currentUser.username}</p>
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
                    <div className="max-w-[1200px] mx-auto space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {[
                                { level: 'LOW', count: tickets.filter(t => (t.priority || '').toUpperCase() === 'LOW').length, color: 'text-emerald-500', desc: 'Prioritas Rendah' },
                                { level: 'Medium', count: tickets.filter(t => (t.priority || '').toUpperCase() === 'MEDIUM').length, color: 'text-amber-500', desc: 'Prioritas Sedang' },
                                { level: 'HIGH', count: tickets.filter(t => (t.priority || '').toUpperCase() === 'HIGH').length, color: 'text-rose-500', desc: 'Prioritas Tinggi' }
                            ].map((item, idx) => (
                        ...
                        <div className="w-full flex h-[62px] shadow-sm rounded-2xl overflow-hidden font-extrabold text-[13px] bg-white border border-slate-100">
                            ...
                            {/* STEP 1: WAITING */}
                            <div 
                                className="w-1/5 bg-rose-50/70 border-r border-rose-100/50 z-50 flex items-center justify-center gap-2.5 hover:bg-rose-100/40 transition-colors cursor-pointer" 
                                style={{ clipPath: 'polygon(0 0, 92% 0, 100% 50%, 92% 100%, 0 100%)' }}
                            >
                                <span className="w-6 h-6 rounded-full bg-rose-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.status === 'On Checking').length}</span> 
                                <span className="text-rose-700 tracking-wide hidden sm:block">Waiting</span>
                            </div>

                            {/* STEP 2: PROGRESS */}
                            <div 
                                className="w-1/5 bg-blue-50/70 border-r border-blue-100/50 z-40 flex items-center justify-center gap-2.5 -ml-4 pl-4 hover:bg-blue-100/40 transition-colors cursor-pointer" 
                                style={{ clipPath: 'polygon(0 0, 92% 0, 100% 50%, 92% 100%, 0 100%, 8% 50%)' }}
                            >
                                <span className="w-6 h-6 rounded-full bg-blue-600 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.status === 'In Progress').length}</span> 
                                <span className="text-blue-700 tracking-wide hidden sm:block">Progress</span>
                            </div>
                            
                            {/* STEP 3: RECHECK */}
                            <div 
                                className="w-1/5 bg-purple-50/70 border-r border-purple-100/50 z-30 flex items-center justify-center gap-2.5 -ml-4 pl-4 hover:bg-purple-100/40 transition-colors cursor-pointer" 
                                style={{ clipPath: 'polygon(0 0, 92% 0, 100% 50%, 92% 100%, 0 100%, 8% 50%)' }}
                            >
                                <span className="w-6 h-6 rounded-full bg-purple-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.status === 'Recheck').length}</span> 
                                <span className="text-purple-700 tracking-wide hidden sm:block">Recheck</span>
                            </div>

                            {/* STEP 4: COMPLETE */}
                            <div 
                                className="w-1/5 bg-emerald-50/70 border-r border-emerald-100/50 z-20 flex items-center justify-center gap-2.5 -ml-4 pl-4 hover:bg-emerald-100/40 transition-colors cursor-pointer" 
                                style={{ clipPath: 'polygon(0 0, 92% 0, 100% 50%, 92% 100%, 0 100%, 8% 50%)' }}
                            >
                                <span className="w-6 h-6 rounded-full bg-emerald-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.status === 'Completed').length}</span> 
                                <span className="text-emerald-700 tracking-wide hidden sm:block">Complete</span>
                            </div>

                            {/* STEP 5: REOPEN */}
                            <div 
                                className="w-1/5 bg-amber-50/60 z-10 flex items-center justify-center gap-2.5 -ml-4 pl-4 hover:bg-amber-100/40 transition-colors cursor-pointer" 
                                style={{ clipPath: 'polygon(0 0, 100% 0, 100% 100%, 0 100%, 8% 50%)' }}
                            >
                                <span className="w-6 h-6 rounded-full bg-amber-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.status === 'Reopen').length}</span> 
                                ...
                                {tickets.map((t, index) => (
                                    ...
                                    >
                                        <div className="w-full sm:w-[30%] mb-3 sm:mb-0">
                                            <p className="font-extrabold text-slate-800 text-[14px] group-hover:text-blue-600 transition-colors">No Task {t.id}</p>
                                        ...
                                        <div className="w-full sm:w-[20%] mb-3 sm:mb-0">
                                            <span className={`inline-flex items-center px-3 py-1 rounded-full text-[11px] font-bold border shadow-sm ${
                                                t.status === 'Completed' ? 'bg-emerald-50 text-emerald-600 border-emerald-150' :
                                                    t.status === 'Reopen' ? 'bg-amber-50 text-amber-600 border-amber-150' :
                                                        t.status === 'In Progress' ? 'bg-blue-50 text-blue-600 border-blue-150' : 'bg-rose-50 text-rose-600 border-rose-150'
                                            }`}>
                                                <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                                                    t.status === 'Completed' ? 'bg-emerald-500' :
                                                        t.status === 'Reopen' ? 'bg-amber-500' :
                                                            t.status === 'In Progress' ? 'bg-blue-500' : 'bg-rose-500'
                                                }`}></span>
                                                {t.status}
                                            </span>
                                        </div>
                                        ...
                                        <div className="w-full sm:w-[15%] mb-3 sm:mb-0">
                                            <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black tracking-wider uppercase ${
                                                (t.priority || '').toUpperCase() === 'LOW' ? 'bg-emerald-100/70 text-emerald-700' :
                                                    (t.priority || '').toUpperCase() === 'MEDIUM' ? 'bg-amber-100/70 text-amber-700' : 'bg-rose-100/70 text-rose-700'
                                            }`}>
                                                {t.priority}
                                            </span>
                                        </div>
                                        ...
                                        <div className="w-full sm:w-[20%] flex items-center justify-start sm:justify-end gap-2.5">
                                            <span className="font-extrabold text-slate-600 text-xs truncate max-w-[110px]">{t.tech}</span>
                                            {t.avatarImg ? (
                                                ...
                                                <div className={`w-8 h-8 rounded-full flex shrink-0 items-center justify-center text-xs font-bold text-white shadow-md ring-2 ring-white/80 ${t.avatar}`}>
                                                    {t.tech.charAt(0)}
                                                </div>
                                            )}
                                        </div>
