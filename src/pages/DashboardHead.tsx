...
import { calculateDynamicPriority, getPriorityWeight, getPriorityBadgeStyle } from '../utils/ticketUtils';
                ...
                <div className="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
                    <div className="max-w-[1200px] mx-auto space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {[
                                { level: 'LOW', count: tickets.filter(t => calculateDynamicPriority(t.date, t.status, t.completedAt, t.reopenCount > 0) === 'LOW').length, color: 'text-emerald-500', desc: 'Prioritas Rendah' },
                                { level: 'Medium', count: tickets.filter(t => calculateDynamicPriority(t.date, t.status, t.completedAt, t.reopenCount > 0) === 'MEDIUM').length, color: 'text-amber-500', desc: 'Prioritas Sedang' },
                                { level: 'HIGH', count: tickets.filter(t => calculateDynamicPriority(t.date, t.status, t.completedAt, t.reopenCount > 0) === 'HIGH').length, color: 'text-rose-500', desc: 'Prioritas Tinggi' }
                            ].map((item, idx) => (
                        ...
                        <div className="w-full flex h-[62px] shadow-sm rounded-2xl overflow-hidden font-extrabold text-[13px] bg-white border border-slate-100">
                            ...
                            >
                                <span className="w-6 h-6 rounded-full bg-rose-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.status === 'Assigned').length}</span> 
                            ...
                            >
                                ...
                                <span className="text-blue-700 tracking-wide hidden sm:block">In Progress</span>
                            </div>
                            ...
                            <div 
                                ...
                                style={{ clipPath: 'polygon(0 0, 92% 0, 100% 50%, 92% 100%, 0 100%, 8% 50%)' }}
                            >
                                <span className="w-6 h-6 rounded-full bg-purple-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.status === 'On Check').length}</span> 
                            ...
                            >
                                <span className="w-6 h-6 rounded-full bg-amber-500 text-white flex items-center justify-center text-xs font-bold shadow-sm">{tickets.filter(t => t.reopenCount > 0).length}</span> 
                        ...
                        <div className="bg-white/90 backdrop-blur-md rounded-[32px] shadow-[0_10px_30px_rgba(0,0,0,0.02)] border border-slate-100 overflow-hidden">
                            ...
                            <div className="divide-y divide-slate-100/80">
                                {[...tickets].sort((a, b) => getPriorityWeight(calculateDynamicPriority(b.date, b.status, b.completedAt, b.reopenCount > 0), b.status) - getPriorityWeight(calculateDynamicPriority(a.date, a.status, a.completedAt, a.reopenCount > 0), a.status)).map((t, index) => (
                                    <div 
                                        key={index} 
                                    ...
                                    >
                                        ...
                                        <div className="w-full sm:w-[15%] mb-3 sm:mb-0">
                                            <span className={`px-2.5 py-1 rounded-lg text-[10px] font-black tracking-wider uppercase ${getPriorityBadgeStyle(calculateDynamicPriority(t.date, t.status, t.completedAt, t.reopenCount > 0), t.status)}`}>
